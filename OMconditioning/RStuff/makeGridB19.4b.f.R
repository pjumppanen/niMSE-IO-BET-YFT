#
# makeGridB19.4.R
# create BET grid B19.4 = full 288 model grid from the reduced B19.1
# The fractional factorial calculation is not automated, those bits are hard-coded
# require(planor)
# require(MASS)

makeGridB19.4.f <- function (path='H:\\C-offline\\MSE-IO-BET-YFT\\OMconditioning\\BET\\', gridName= 'gridB19.4', makeGrid=T, makeBatFiles=T, splitMasterBatch=32, batFile="projBatNoHessJitterLoop")
{
...
#estimate=  ~ (h.val + M.val + t.val + q.val + i.val + iCV.val +                          ess.val)^2  , # main effects only
    ...
    nunits= prod(nlevels)/(1), # (set denominator to 1 for full grid)
...
makeGridB19.4.f(path=path, makeBatFiles=F, makeGrid=F)
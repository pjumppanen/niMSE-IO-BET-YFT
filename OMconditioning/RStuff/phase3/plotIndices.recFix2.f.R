                          ...
                          cpueNormYrs=NULL)
{
  ...
  for (i in 1:length(modList))
  {
    if (modList[i] != "nullMod")
    {
      ...
      #couple BET test cases to check consistency with projection.R
      if (modList[i] %in% c("h70_M08_t01_q1_iR1_ess10_SL", "h70_M06_t0001_q1_iR1_ess10_SL")){
         plot(cpueAgg$cpue, ylim=c(0,5), main=modList[i] %&% "\n" %&% MPcpueRMSE)
         lines(cpueAgg$cpuePred)
         print(c(MPcpueRMSE, MPcpueAC))
      }

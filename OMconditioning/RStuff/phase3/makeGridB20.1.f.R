...
makeGridB20.1.f <- function (path='H:\\C-offline\\MSE-IO-BET-YFT\\OMconditioning\\BET\\', gridName= 'gridB20.1', makeGrid=T, makeBatFiles=T, splitMasterBatch=35, batFile="projBatNoHessJitterLoop")
{
...
browser()
      ...
      if(makeGrid){
         ...
         iMatBase <- read.table(path  %&% gridName %&% "\\gridTemplate\\betOMcpue2019.dat", header=T, stringsAsFactors=F)
...
deleteMe <- makeGridB20.1.f(path=path, makeBatFiles=F, makeGrid=F)
print(deleteMe)

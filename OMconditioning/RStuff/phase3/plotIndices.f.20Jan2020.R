                          ...
                          MPcpue=NA)
{
  ...
  for (i in 1:length(modList))
  {
    if (modList[i] != "nullMod")
    {
      ...
      cpueNormYr1 <- 1980      #2016#1980
      cpueNormYr2 <- 2018
      cpueAgg$cpue    <- cpueAgg$cpue/mean(cpueAgg$cpue[cpueAgg$yr %in% (cpueNormYr1:cpueNormYr2)]) 
      cpueAgg$cpuePred <- cpueAgg$cpuePred/mean(cpueAgg$cpuePred[cpueAgg$yr %in% (cpueNormYr1:cpueNormYr2)])
  
      cpueAgg$dev  <- log(cpueAgg$cpue) - log(cpueAgg$cpuePred)
      MPcpueRMSE <- sqrt(mean(cpueAgg$dev^2))

      #remove missing observation effect from AC calc
      seq <- cpueAgg$yr[cpueAgg$yr[2:(length(cpueAgg$yr)-1)] == cpueAgg$yr[1:(length(cpueAgg$yr)-2)] + 1]
      MPcpueAC   <- cor(cpueAgg$dev[cpueAgg$yr %in% seq], cpueAgg$dev[cpueAgg$yr %in% (seq+1)])
  ...
  return (list(cbind(CPUE.fit, CL.Fit, tag.Fit, max.Grad, rec.Trend, rec.Fit, rec.RMSE,recByYr.RMSE, MSY, B_B.MSY, B_B0, BY, BMSY, modList, stdExistsall, crashPen, catchLLH, cpueMPRMSEByYrall, cpueMPACByYrall), boundFailLOList, boundFailHIList, boundFailHIList2))
}

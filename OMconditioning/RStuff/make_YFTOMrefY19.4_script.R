#=============================================================================================================================
# Basic R script for making and inspecting the conditioned YFT Operating Model OMrefY19.4 and associated robustness scenarios
# Sep 2019
# Basically the same as OMrefY19.3, except it explores issues of fractional frid design and minimization sensitivity
#=============================================================================================================================
#online
mainRootDir <- "H:\\C-offline\\MSE-IO-BET-YFT\\"  #modify for local path
gitMirrorDir <- "H:\\C-offline\\MSE-IO-BET-YFT\\gitMirror\\phase2\\niMSE-IO-BET-YFT\\" 
...
##################################################################################
# gridY19.1 - minimal fractional factorial new template test
path=OMRootDir %&% 'OMconditioning\\YFT\\'
source(mainRootDir %&% "OMconditioning\\RStuff\\phase2\\makeGridY19.1.f.R")
OMgridY19.1.List2 <- makeGridY19.1.f(path=path, gridName= 'gridY19.1', makeGrid=F, makeBatFiles=T, splitMasterBatch=60, batFile="projBatNoHessJitterLoop")    #check the list of grid elements, but don't create the grid DIR structure
OMgridY19.1.List <- makeGridY19.1.f(path=path, gridName= 'gridY19.1', makeGrid=T, makeBatFiles=T, splitMasterBatch=20, batFile="projBatNoHessJitterLoop")    #check the list of grid elements, but don't create the grid DIR structure
print(OMgridY19.1.List)


#make the list into a uniformaly weighted list with OM specs as names
names(OMgridY19.1.List) <- OMgridY19.1.List
OMgridY19.1.List[] <- 1/length(OMgridY19.1.List)
print(OMgridY19.1.List)

save(OMgridY19.1.List,file = mainRootDir %&% "gitMirror\\phase2\\niMSE-IO-BET-YFT\\Objects\\"  %&% "OMgridY19.1.List.RDA")
load(file = mainRootDir %&% "gitMirror\\phase2\\niMSE-IO-BET-YFT\\Objects\\"  %&% "OMgridY19.1.List.RDA")

...
importGrid.f <- function(
    ...
    keepEverything=T, #keepEverything added to save re-importing everytime, uses too much memory to do a big list
      ...
      if(doLoad & !exists(gridList[i])){   #don't reload if it already exists in memory
        ...
        for(iCon in 1:convergedNum){
          ...
          if(LLH < bestOBJfn) {
            ...
            if(keepEverything){
              best$M <- M
              best$stdExists <- stdExists
            } else {
              best <- list(tmp$derived_quants, tmp$recruit, tmp$maximum_gradient_component,tmp$likelihoods_used,tmp$likelihoods_raw_by_fleet,tmp$cpue, tmp$Length_comp_Eff_N_tuning_check, tmp$timeseries[,1:8], tmp$parameters[,1:9],M,
                 tmp$equil_yield, stdExists )
              names(best) <- c('derived_quants', 'recruit', 'maximum_gradient_component','likelihoods_used',"likelihoods_raw_by_fleet",'cpue', 'Length_comp_Eff_N_tuning_check', 'timeseries', 'parameters','M',
                'equil_yield', 'stdExists')
            }
          }
        } #iCon
        assign(gridList[i],best, envir = .GlobalEnv)
      }
    }
  }
  print(loadList)

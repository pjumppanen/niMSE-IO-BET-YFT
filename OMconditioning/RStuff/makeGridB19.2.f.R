#
# makeGridB19.2.R
# create BET grid B19.2 = 36 model fractional grid from 288 model full frid B19.1
# The fractional factorial calculation is not automated, those bits are hard-coded
# require(planor)
# require(MASS)

makeGridB19.2.f <- function (path='H:\\C-offline\\MSE-IO-BET-YFT\\OMconditioning\\BET\\', gridName= 'gridB19.2', makeGrid=T, makeBatFiles=T, splitMasterBatch=T, batFile="projBatNoHessJitterLoop")
{
...
#model    = ~(  h.val + M.val + t.val + q.val + i.val + iCV.val +                        ess.val  )^2,
    model    = ~(  h.val + M.val + t.val + q.val + i.val + iCV.val +                        ess.val  ),
...
makeGridB19.2.f(path=path, makeBatFiles=F, makeGrid=F)
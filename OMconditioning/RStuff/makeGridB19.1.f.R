#
# makeGridB19.1.R
# create BET grid B19.1 = reference case BET OM grid from WPM/WPTT 2018 using fractional factorial design
# The fractional factorial calculation is not automated, those bits are hard-coded
# Decided to drop "t01" because fractionation options for 3 level dimensions seems much more limited than 2 level options
# require(planor)
# require(MASS)

makeGridB19.1.f <- function (path='H:\\C-offline\\MSE-IO-BET-YFT\\OMconditioning\\BET\\', gridName= 'gridB19.1', makeGrid=T, makeBatFiles=T, splitMasterBatch=T, batFile="projBatNoHessJitterLoop")
{
...
#            }
            ...
            if(makeBatFiles){
              ...
              if(splitMasterBatch){
                if(iMod <= 0.1*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf01, sep='\n' )
                if(iMod  > 0.1*length(gridList) & iMod <= 0.20*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf02, sep='\n' )
                if(iMod  > 0.2*length(gridList) & iMod <= 0.30*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf03, sep='\n' )
                if(iMod  > 0.3*length(gridList) & iMod <= 0.40*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf04, sep='\n' )
                if(iMod  > 0.4*length(gridList) & iMod <= 0.50*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf05, sep='\n' )
                if(iMod  > 0.5*length(gridList) & iMod <= 0.60*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf06, sep='\n' )
                if(iMod  > 0.6*length(gridList) & iMod <= 0.70*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf07, sep='\n' )
                if(iMod  > 0.7*length(gridList) & iMod <= 0.80*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf08, sep='\n' )
                if(iMod  > 0.8*length(gridList) & iMod <= 0.90*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf09, sep='\n' )
                if(iMod  > 0.9*length(gridList)) cat("\ncd " %&% modName %&% projCall %&% "\ncd .. \n", file=mf10, sep='\n' )
              }
            } #if makeBatFiles
       } # end iMod loop
#        }

...
# --- Description --------------------------------------------------------------
MseDef@Name          <- "OMrefB19.0.288"
MseDef@Label         <- "OMrefB19.0.288" #useful for changing graphics labels
MseDef@Date          <- "Jan2019"
MseDef@Author        <- "D.Kolody"
MseDef@Notes         <- "BET reference case OM from OMrefB19.0 (re-done), fractional factorial screening grid of 128 models "
...
MseDef@SSRootDir     <- "Z:\\MSE-IO-BET-YFT\\OMconditioning\\BET\\gridB19.0\\"          # Root dir for SS results outputs
MseDef@SBlim         <- 0.5
MseDef@Flim          <- 1.3


load("objects\\OMgridB19.0.List.RDA")       #the weighted list of BET models
MseDef@OMList       <- as.list(names(OMgridB19.0.List))

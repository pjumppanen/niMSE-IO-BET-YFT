...
# --- Create a blank MseDefinition object -------------------------------------
MseDef <- new('MseDefinition')

# --- Description -------------------------------------------------------------
MseDef@Name          <- "OMyftNEr"
MseDef@Label         <- "OMyftNEr" #useful for changing graphics labels
MseDef@Date          <- "Jun 2016"
MseDef@Author        <- "D.Kolody"
MseDef@Notes         <- "OM based on 2 SS specifications without stochastic variability; 1 rep per SS configuration, R projections"
MseDef@PrimarySource <- "YFT Test case"
...
MseDef@proyears      <- as.integer(50)                                                # Number projection years
...
MseDef@catchBridge    <- as.karray(c(1))        # known catch history between last assessment year in OM and firstMPYr (length of min 0 to max firstMPYr-lastCalendarYr-1)
...
MseDef@ReccvTin     <- array(rep(0.,MseDef@npop), dim=MseDef@npop)  # Temporal variability in recruitment aggregate over regions
...
MseDef@selExpRange  <- 0.                     # sel temporal variability exponent - oscillates with a sin wave rangeing between +/- exp(selExpRange)
MseDef@selAgeRange  <- 0.                     # 0=no age shift, 2 means (discretized) sine wave shift of sel vector between - 2 and + 2 age class

...
# --- Description --------------------------------------------------------------
MseDef@Name          <- "yftY19.4.500"
MseDef@Label         <- "yftY19.4.500" #useful for changing graphics labels
MseDef@Date          <- "Sep2019"
MseDef@Author        <- "D.Kolody"
MseDef@Notes         <- "420 model YFT reference set from OMgridY19.4, with catchLLH and repeat convergence filtering and CPUE CV<0.3"
MseDef@PrimarySource <- "higher order fractional than OMref19.3 used for TCMP 2019."
...
MseDef@SSRootDir     <- "E:\\KOL018\\MSE-IO-BET-YFT\\OMconditioning\\YFT\\gridY19.3.2way\\"          # Root dir for SS results outputs
MseDef@SBlim         <- 0.4
MseDef@Flim          <- 1.4
...
MseDef@OMList       <- as.list(names(OMrefY19.4List))
MseDef@modelWeight  <- as.karray(OMrefY19.4List)  #= "karray",    # Plausibility weighting for given OMFile

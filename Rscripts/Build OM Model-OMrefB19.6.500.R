...
# ============================================================================================================================================
# OMrefB19.6.500 - the reference set 90 model, 500 realization BET OM prepared for the 2019 WPTT
# variability in selectivity not active
# implementation error not active
# bridging catch from 2017 implemented
...
MseDef@SSRootDir     <- "E:\\KOL018\\MSE-IO-BET-YFT\\OMconditioning\\BET\\gridB19.6\\"          # Root dir for SS results outputs
MseDef@SBlim         <- 0.5
MseDef@Flim          <- 1.3


load("objects\\OMrefB19.6List.RDA")       #the weighted list of BET models
MseDef@OMList       <- as.list(names(OMrefB19.6List))

MseDef@OMList       <- as.list(names(OMrefB19.6List))
MseDef@modelWeight  <- as.karray(OMrefB19.6List)  #= "karray",    # Plausibility weighting for given OMFile

...
# ============================================================================================================================================
# OMrefB19.5.288 - a 288 model, 288 realization BET OM prepared for the 2019 MSE task force
...
# --- Description --------------------------------------------------------------
MseDef@Name           <- "OMrefB19.5.288"
MseDef@Label          <- "OMrefB19.5.288" #useful for changing graphics labels
MseDef@Date           <- "Jan2019"
MseDef@Author         <- "D.Kolody"
MseDef@Notes          <- "BET reference case OM from OMrefB19.5, full factorial grid 288 models "
...
MseDef@SSRootDir      <- "Z:\\MSE-IO-BET-YFT\\OMconditioning\\BET\\gridB19.5\\"          # Root dir for SS results outputs
MseDef@SBlim          <- 0.5
MseDef@Flim           <- 1.3


load("objects\\OMgridB19.5.List.RDA")       #the weighted list of BET models
MseDef@OMList         <- as.list(names(OMgridB19.5.List))
...
MseDef@ReccvTin     <- array(rep(0.42,MseDef@npop), dim=MseDef@npop) # Temporal variability in recruitment aggregate over regions
...
MseDef@IACin      <- 0.5              # series-specific AC corresponds to annual aggregate AC = 0.5
MseDef@Icv        <- c(0.2, 0.20001)  # corresponds to annual CV of 0.2

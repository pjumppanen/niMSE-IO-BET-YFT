SS_output.f <- function (dir = "C:/myfiles/mymodels/myrun/", model = "ss3", 
    ...
    hidewarn = FALSE, NoCompOK = FALSE, aalmaxbinrange = 4) 
{
    ...
    if (SS_versionshort == "SS-V3.11") {
        ...
        FleetNames <- matchfun2("FleetNames", 1, "FleetNames", 
            nfleets, cols = 2)
        ...
        rawdefs <- matchfun2("DEFINITIONS", 1, "LIKELIHOOD", 
            -1)
    ...
    if (comp) {
        ...
        lbins <- NA
        nlbins <- NA
        temp <- rawrep[grep("NUMBERS_AT_LENGTH", rawrep[, 1]) + 
            1, ]
    ...
    stats$Run_time <- paste(as.character(matchfun2("StartTime", 
    ...
    stats$likelihoods_raw_by_fleet <- matchfun2("Fleet:", 0, 
    ...
    if (covar) {
        ...
        if (checkcor == TRUE & stats$N_estimated_parameters > 
            ...
            if (verbose) 
                cat("You skipped the correlation check\n")
        }
    }
    else {
        if (verbose) 
    ...
    for (icol in 2:5) rmse_table[, icol] <- as.numeric(rmse_table[, 
        icol])
    ...
    lenntune$"MeaneffN/MeaninputN"[lenntune$"MeaneffN/MeaninputN" == 
        "-1.#IND"] <- NA
    for (i in 2:ncol(lenntune)) lenntune[, i] <- as.numeric(lenntune[, 
        i])
    ...
    if (recruitment_dist$Used[spawnseas] == 0) 
        temp <- morph_indexing[morph_indexing$Bseas == min(recruitment_dist$Seas[recruitment_dist$Used == 
            1]) & morph_indexing$Sub_Morph_Dist == max(morph_indexing$Sub_Morph_Dist), 
            ]
    mainmorphs <- min(temp$Index[temp$Gender == 1])
    if (nsexes == 2) 
        mainmorphs <- c(mainmorphs, min(temp$Index[temp$Gender == 
            2]))
    ...
    if (SS_versionNumeric < 3.2) {
        ...
        for (icol in 1:3) discard_spec[, icol] <- as.numeric(discard_spec[, 
            icol])
    ...
    #DK quick and dirty bug fix - may not work for all cases...
    #if (nrow(Z_at_age) > 0) {
    if (!is.null(Z_at_age)) {

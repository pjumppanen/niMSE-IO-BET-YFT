...
setMethod("initialize", "StockSynthesisModel",
  function(.Object, MseDef, which, seed, Report=FALSE, UseMSYss=0)
  {
...
setMethod("msevizHistoricTimeSeriesData", c("StockSynthesisModel"),
  function(.Object, mseFramework, plotByRF, df)
  {
    if (class(mseFramework) != "MseFramework")
    {
      print(paste("ERROR: Could not extract time series data.",deparse(substitute(MseFramework)),"not of class MseFramework"))
      stop()
    }

    extractData <- function(HistoricVars, RefVars)
    {
      Yrs  <- mseFramework@MseDef@firstCalendarYr + (0:(HistoricVars@nyears - 1))
      C1   <- Yrs
      C3   <- rep(.Object@ModelData@which, times=length(Yrs))
      df   <- NULL

      addRows <- function(df, data, name)
      {
        C2 <- as.array(data)
      ...
      df         <- addRows(df, HistoricVars@IobsArchive, "CPUE(aggregate)")

      # Recruitment
      df         <- addRows(df, HistoricVars@Rec, "Recruitment")

      #B/B0
      B_B0       <- b_b0(HistoricVars, RefVars)
      df         <- addRows(df, apply(B_B0, MARGIN=c(2), FUN=sum), "B/B0")

      #B/BMSY
      B_BMSY     <- b_bmsy(HistoricVars, RefVars)
      df         <- addRows(df, B_BMSY, "B/BMSY")

      #SSB/SSB0
      SSB_SSB0   <- ssb_ssb0(HistoricVars, RefVars)
      df         <- addRows(df, apply(SSB_SSB0, MARGIN=c(2), FUN=sum), "SSB/SSB0")

      #SSB/SSBMSY
      SSB_SSBMSY <- ssb_ssbmsy(HistoricVars, RefVars)
      df         <- addRows(df, SSB_SSBMSY, "SSB/SSBMSY")

      #F/FMSY
      F_FMSY     <- f_fmsy(HistoricVars, RefVars)
      df         <- addRows(df, F_FMSY, "F/FMSY")

      #Catch
      df         <- addRows(df, apply(HistoricVars@CM, MARGIN=c(2), FUN=sum), "C")

      # Rec by Qtr
      QtrYrBases <- (0:(HistoricVars@nyears-1)) * 4

      df <- addRows(df, HistoricVars@RecYrQtr[QtrYrBases + 1], "Recruitment Q1")
      df <- addRows(df, HistoricVars@RecYrQtr[QtrYrBases + 2], "Recruitment Q2")
      df <- addRows(df, HistoricVars@RecYrQtr[QtrYrBases + 3], "Recruitment Q3")
      df <- addRows(df, HistoricVars@RecYrQtr[QtrYrBases + 4], "Recruitment Q4")
      ...
      if (plotByRF)
      {
        ...
        if (HistoricVars@nfleets > 1)
        {
          ...
          {
            df <- addRows(df, HistoricVars@CMbyF[mseFramework@MseDef@targpop,,fi], paste("C by Fleet",fi))
          }
        }
        ...
        if (HistoricVars@nareas > 1)
        {
          ...
          {
            df <- addRows(df, HistoricVars@IobsRArchive[,ri], paste("CPU by Area",ri))
          }
        }
      }
    ...
    Results <- rbind(data.frame(df), data.frame(extractData(.Object@HistoricVars, .Object@RefVars)))
...
setMethod("msevizProjectedTimeSeriesData", c("StockSynthesisModel"),
  function(.Object, mseFramework, plotByRF, df)
  {
    if (class(mseFramework) != "MseFramework")
    {
    ...
    extractData <- function(ProjectedVars, RefVars)
    {
      SY   <- as.matrix(expand.grid(nsims=1:ProjectedVars@nsim, nyrs=1:ProjectedVars@nyears))
      Yrs  <- mseFramework@MseDef@firstCalendarYr + .Object@ModelData@nyears + (0:(ProjectedVars@nyears - 1))
      C1   <- rep(ProjectedVars@MP, times=length(SY[,1]))
      C2   <- Yrs[SY[,2]]
      C4   <- SY[,1]
      df   <- NULL
      ...
      # Aggregate CPUE series skips final year
      df         <- addRows(df, ProjectedVars@IobsArchive, "CPUE(aggregate)")

      # Recruitment
      df         <- addRows(df, ProjectedVars@Rec, "Recruitment")

      #B/B0
      B_B0       <- b_b0(ProjectedVars, RefVars)
      df         <- addRows(df, apply(B_B0, MARGIN=c(1,3), FUN=sum), "B/B0")

      #B/BMSY
      B_BMSY     <- b_bmsy(ProjectedVars, RefVars)
      df         <- addRows(df, B_BMSY, "B/BMSY")

      #SSB/SSB0
      SSB_SSB0   <- ssb_ssb0(ProjectedVars, RefVars)
      df         <- addRows(df, apply(SSB_SSB0, MARGIN=c(1,3), FUN=sum), "SSB/SSB0")

      #SSB/SSBMSY
      SSB_SSBMSY <- ssb_ssbmsy(ProjectedVars, RefVars)
      df         <- addRows(df, SSB_SSBMSY, "SSB/SSBMSY")

      #F/FMSY
      F_FMSY     <- f_fmsy(ProjectedVars, RefVars)
      df         <- addRows(df, F_FMSY, "F/FMSY")

      #Catch
      df         <- addRows(df, apply(ProjectedVars@CM, MARGIN=c(1,3), FUN=sum), "C")

      # Rec by Qtr
      QtrYrBases <- (0:(ProjectedVars@nyears-1)) * 4

      df <- addRows(df, ProjectedVars@RecYrQtr[,QtrYrBases + 1], "Recruitment Q1")
      df <- addRows(df, ProjectedVars@RecYrQtr[,QtrYrBases + 2], "Recruitment Q2")
      df <- addRows(df, ProjectedVars@RecYrQtr[,QtrYrBases + 3], "Recruitment Q3")
      df <- addRows(df, ProjectedVars@RecYrQtr[,QtrYrBases + 4], "Recruitment Q4")

      if (plotByRF)
      {
        #Catch by fishery
        if (ProjectedVars@nfleets > 1)
        {
          for (fi in 1:ProjectedVars@nfleets)
          {
            df <- addRows(df, ProjectedVars@CMbyF[,mseFramework@MseDef@targpop,,fi], paste("C by Fleet",fi))
          }
        }

        #CPUE by region
        if (ProjectedVars@nareas > 1)
        {
          for (ri in 1:ProjectedVars@nareas)
          {
            df <- addRows(df, ProjectedVars@IobsRArchive[,,ri], paste("CPU by Area",ri))
          }
        }
      }
    ...
    for (ProjVar in .Object@ProjectedVars)
    {
      df <- rbind(data.frame(df), data.frame(extractData(ProjVar, .Object@RefVars)))
    }

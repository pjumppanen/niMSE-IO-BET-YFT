...
PellaTomlinsonProjection <- function(pset, CMaxProp=1.0, Gain=0.15, MinCatchProp=0.15, debug=FALSE)
{
  ...
  else
  {
    ...
    maxp[1]    <- params[1] + log(20)
    minp[1]    <- params[1] - log(20)
    maxp[2]    <- params[2] + log(20)
    minp[2]    <- params[2] - log(20)
    maxp[3]    <- min(log(1.0), params[3] + log(1.25))
    minp[3]    <- max(log(0.1), params[3] - log(1.25))
  }
  ...
  if (length(pset$env$K) > 0)
  {
    ...
    if ((span > 10) || (span < 0.1))
    {
      ...
      print("poor fit so using last good fit")
    }
    else
    {
...
#  lastTAC <- pset$prevTACE$TAC / CMaxProp
  ...
  # TAC of lastTAC
  BStart  <- PT.project.2(Fit, rep(lastTAC, times=endTime), Fit$BY)[endTime]
#  Btarget <- BStart + ((Fit$BMSY - BStart) * R)
  Btarget <- BStart + ((CMaxProp * Fit$BMSY - BStart) * R)

  projection.objective <- function(TAC)
  {
     obj <- (Btarget - (PT.project.2(Fit, rep(TAC, times=endTime), BStart)[endTime])) ^ 2
...
#  IStart  <- BStart * Fit$q
#  Itarget <- IStart + ((CMaxProp * Fit$CPUE_MSY - IStart) * R)
#
#  projection.objective <- function(par, Fit, lastTAC, endTime, BStart, Itarget)
#  {
#    Scale <- exp(par)
#    Bend  <- PT.project(Fit, rep(Scale * lastTAC, times=endTime), BStart)[endTime]
#    obj   <- (Itarget - (Bend * Fit$q)) ^ 2
#
#    return (obj)
#  }
#
#  # if F is unrealistically high the catch limiting can cause a local maximum
#  # at high F so we make sure we have a low F as as a starting point
#  TAC_opt <- nlminb(start=log(1.0e-2), objective=projection.objective, Fit=Fit, lastTAC=lastTAC, endTime=endTime, BStart=BStart, Itarget=Itarget, lower=log(1e-6), upper=log(1e+3))
#  newTAC  <- lastTAC * exp(TAC_opt$par)
...
#  }
#
#  ix       <- ((length(I_hist) - 3):length(I_hist))
#  Itarget  <- CMaxProp * Fit$CPUE_MSY
#  Iactual  <- mean(I_hist[ix])
#  deltaTAC <- 0.1 * (Iactual - Itarget) / Fit$q
#
#  newTAC <- lastTAC + deltaTAC
  ...
  if (debug || TRUE)
  {
#    print(paste("r=", Fit$r, "K=", Fit$K, "p=", Fit$p, "q=", Fit$q, "n=", Fit$n, "MSY=", Fit$MSY, "CPUE_MSY=",Fit$CPUE_MSY, "Iactual=", Iactual, "ITarget=", Itarget, "TAC=", newTAC, "lastTAC=", lastTAC))
#    print(paste("r=", Fit$r, "K=", Fit$K, "p=", Fit$p, "q=", Fit$q, "n=", Fit$n, "MSY=", Fit$MSY, "CPUE_MSY=",Fit$CPUE_MSY, "IStart=", IStart, "ITarget=", Itarget, "TAC=", newTAC, "lastTAC=", lastTAC))
...
MP_FunctionExports <- c(MP_FunctionExports, "PT.project.2")

# -----------------------------------------------------------------------------

PT.project.2 <- function(model, C, B0)
{
  Y     <- length(C)
  r     <- model$r
  K     <- model$K
...
PT.project <- function(model, C, B0)
{
  ...
  for(y in 2:Y)
  {
    ...
    # impose an asymptotic limit to 10000
    limit <- 100.0
    if (B[y] < 3 * limit)
    {
...
MP_FunctionExports <- c(MP_FunctionExports, "PT.model.fixed.p.msy")


# -----------------------------------------------------------------------------
# Pella-Tomlinson Model function
# -----------------------------------------------------------------------------
PT.model.fixed.p.msy <- function(params, C_hist, I_hist, Type, p, msy, weight=NULL)
{
  return (PT.model(c(params[1], log(msy), log(p), params[2]), C_hist, I_hist, Type, weight))
}
...
PT.model <- function(params, C_hist, I_hist, Type, weight=NULL)
{
  ...
  for(y in 2:Y)
  {
    B[y] <- B[y-1] + (r / p) * B[y-1] - (r / p) * B[y-1] * ((B[y-1] / K) ^ p) - C_hist[y-1]
...
# Pella-Tomlinson Model function
# -----------------------------------------------------------------------------
PT.model.2 <- function(params, C_hist, I_hist, Type, weight=NULL)
{
  # Model formulation is,
  # B(t+1)=B(t) + (r/p)B(t)(1-(B(t)/K)^p) - C(t)
  #
...
# !   RW status of diff variables: params:out pt:in-killed
